...
| 工程师 | 6 | 5 | 220 | 慢 | **部署炮台**：放下一座自动射击的炮台，持续 8 秒（冷却 12 秒）。初始武器弹弹枪 |
...
|---|---|---|
| 史莱姆 | 直接冲向玩家 | 1 层起 |
| 蝙蝠 | 飞得快、走 S 形路线，血少 | 1 层起 |
| 哥布林枪手 | 保持距离、绕圈走位，打扇形子弹 | 1 层起 |
| 冲锋野猪 | 原地蓄力（抖动闪红）后直线冲锋，撞墙会晕一会儿，能撞碎木箱 | 1 层起 |
| 分裂史莱姆 | 死后分裂成两只小史莱姆 | 2 层起 |
| 刺球怪 | 定时鼓起身子，向四周放一圈子弹 | 2 层起 |
| 炸弹哥布林 | 向玩家位置抛炸弹，落点先出现红圈预警再爆炸 | 2 层起 |
| 骷髅法师 | 保持距离，定时召唤蝙蝠（最多 4 只） | 3 层起 |
| Boss · 地牢恶魔 | 追击 / 环形弹幕 / 扇形连射循环 | 每层最后 |
...
**画面和光照**：
- 地牢整体压暗（`dungeon.tscn` 里的 `Ambient` 节点，改颜色就能调亮暗），再用光源照亮：玩家身上一盏、墙上的火把（会闪烁）、传送门、枪口火光和爆炸。
- 子弹、掉落物、特效不受光照影响，在暗处也很醒目。
- 墙是 2.5D 的：下方是地板的墙画成砖墙正面，其余画成墙顶，墙根下的地板带阴影（`dungeon.gd` 的 `_paint_tiles()`）。
- 地面会随机撒骨头、杂草、碎石等装饰（`DECOR_CHANCE` 调密度）。
- 角色和敌人有脚下阴影、走路颠动、脚步扬尘，受击会飘伤害数字，屏幕四周有暗角。

**角色动画**：角色贴图是 24×24 的横向动画条，一共 6 帧：前 2 帧是待机（呼吸），后 4 帧是走路。玩家会自动按状态播放。换成自己画的角色时保持同样的排列就行；只有一帧的贴图也能用，会退回到代码颠动。

**换美术**：直接替换 `assets/sprites/` 下的同名 PNG 就行，尺寸可以不同。`tiles.png` 里第一行是地板（4 种花纹 + 墙根阴影），第二行是墙（墙顶 + 2 种砖墙正面），碰撞配置在 `dungeon/tileset.tres`。推荐的免费素材：
